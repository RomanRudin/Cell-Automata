...
class GameSprite(sprite.Sprite):
    def __init__ (self, block_image, block_x, block_y, direction_x, direction_y, health, wcd, is_placed, *args):
        ...
        self.rect[0] += block_x
        self.rect[1] += block_y
        self.cor_x, self.cor_y = self.rect.x // cell_x, self.rect.y // cell_y
    ...
    def start(self):
        wall_checking_data[self.cor_y][self.cor_x] = self
    #methods transforms images for rotated blocks
    def rotated(self, direct):
        if self.direction_x == 1:
            self.direction_x = 0
    ...
    def wcd_checking(self, self_adding, s):
        while True:
            pushed_block = wall_checking_data[self.cor_y + s * self.direction_y][self.cor_x + s * self.direction_x]
            ...
            if (pushed == 0) or (pushed >= 8) or (pushed == -1) or (pushed <= -4):
                ...
                if ((self.direction_x == pushed_block.direction_x) and (self.direction_x != 0)) or ((self.direction_y == pushed_block.direction_y) and (self.direction_y != 0)):
                    wall_check = wall_checking_data[self.cor_y + self.direction_y * (s + 1)][self.cor_x + self.direction_x * (s + 1)]
                    if wall_check.wcd == 9:
                        wall_check.kill()
                        pushed_block.kill()
                        object_generating(self.cor_x + self.direction_x * (s + 1), self.cor_y + self.direction_y * (s + 1), 10, 0, 0, False)
                        s -= 1
        ...
        for i in range(s - 1 + self_adding):
            animation_group.add(wall_checking_data[self.cor_y][self.cor_x])
        ...
        if s > 1 - self_adding:
            prepushed_block = wall_checking_data[self.cor_y + (s - 1) * self.direction_y][self.cor_x + (s - 1) * self.direction_x]
        else:
